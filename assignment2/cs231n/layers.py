...
def affine_forward(x, w, b):
    ...
    x_now=np.reshape(x,(x.shape[0],-1))
    out=np.matmul(x_now,w)
    out=out+b
...
def affine_backward(dout, cache):
    ...
    x_now=np.reshape(x,(x.shape[0],-1))
    #b_tmp=np.ones(dout.shape[0]).reshape(dout.shape[0],1)
    #db=np.matmul(b_tmp.T,dout)
    #print("dout\n",dout)
    #print("right\n",db)
    db=np.sum(dout,axis=0)
    #print("wrong\n",db)
    dw=np.matmul(x_now.T,dout)
    dx_now=np.matmul(dout,w.T)
    dx=dx_now.reshape(x.shape)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    now_mean=np.mean(x,axis=1)
    now_var=np.var(x,axis=1)
    now_std=np.sqrt(now_var+eps)
